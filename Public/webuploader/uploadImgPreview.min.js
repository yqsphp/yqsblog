(function(c) {
	var i = {
		...
		method: "POST",
		chunked:true, //分流上传
		chunkSize: 200*1024*1024*8,//分流上传大小 默认200M，超过该值会将文件分成n等分依次上传
		sendAsBlob: false,
	...
	function a(l, p, o, k) {
		var r = c('<li id="' + l.id + '">' + '<p class="title">' + l.name + "</p>" + '<p class="imgWrap"></p>' + '<p class="progress"><span></span></p>' + "</li>"),
			m = i.showToolBtn ? c('<div class="file-panel">' + '<span class="cancel">删除</span>' + '<span class="rotateRight">向右旋转</span>' + '<span class="rotateLeft">向左旋转</span></div>').appendTo(r) : null,
			...
			n = function(t) {
				...
				j.find(".retry-this").on("click", function() {
					p.retry(l)
				});
				j.find(".del-this").on("click", function() {
					b(l, k);
					p.removeFile(l, true)
				})
			};
		...
		l.on("statuschange", function(u, t) {
			if (t === "progress") {
				q.hide().width(0)
			} else {
				if (t === "queued") {
	...
	function e(q) {
		if (!WebUploader.Uploader.support()) {
			if (i.notSupport && c.isFunction(i.notSupport)) {
				...
				throw new Error("WebUploader不支持该的浏览器。.")
			}
		}
		...
		r.on("uploadAccept", function(w, v) {
		...
		r.on("uploadBeforeSend", function(w, v) {
		...
		//文件上传成功后回调ajax数据
		r.on('uploadSuccess',function(file,res){
			if (i.callback && c.isFunction(i.callback)) {
				i.callback.apply(this, arguments);
			}
		});
		r.onError = function(v) {
			...
			if (v == "F_EXCEED_SIZE") {
				...
				w.msg = "上传的文件中有重复";
			}else if(v == "Q_EXCEED_SIZE_LIMIT"){
				w.msg = "总文件大小超出限制";
			}else if(v == "Q_EXCEED_NUM_LIMIT") {
				w.msg = "上传文件数量超出总数量";
			}else if(v == "Q_TYPE_DENIED") {
				w.msg = "上传的文件类型不满足要求";
			}
